...
* **Key Visualizations & Text Insights Added**:
  ...
  * **Contract Type vs Churn Count**: Proved that month-to-month contracts account for almost all churn (around 42.7%), while two-year contracts are extremely stable (under 3% churn).
...
* **Key Outputs & Evaluation Achievements**:
  ...
  * **Approach 2 & 2 Again (Hyperparameter Sweeping)**: Tested combinations across n_estimators_list and max_depth_list. The optimized balanced configuration (n_estimators=300, max_depth=10) achieved a strong recall of around 80% on the churn class and an overall test accuracy of around 76%.
  ...
  * **Cross-Validation**: Confirmed model stability across different subsets of the data with a Mean CV Accuracy of around 76% and a Mean CV Recall of around 80%.
  * **ROC-AUC Boundary**: Achieved a solid Area Under the Curve score of around 0.84, validating strong model classification power.
